...
public class Selectyuangong extends javax.swing.JFrame {

    public Selectyuangong() {
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        jInternalFrame3.setTitle("员工信息查询");
        jInternalFrame3.setVisible(true);

        jPanel4.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "查询员工信息条件设置", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("宋体", 0, 18), new java.awt.Color(51, 51, 51))); // NOI18N

        xuanJCB2.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "员工编号", "姓名", "电话", "月收入", "归属管理员编号" }));
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "员工编号", "姓名", "性别", "年龄", "电话", "月收入", "入职日期", "归属管理员编号"
            }
        ));
    ...
    private void tiaojianJB2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_tiaojianJB2ActionPerformed
        ...
        try{
            ...
            if(xuan=="员工编号")
            {
                sql="select *  from yuangongtable where  yuangongid='"+this.tiaojianJTF.getText()+"'";
            }
            if(xuan=="姓名")
            {
                sql="select *  from yuangongtable where  name='"+this.tiaojianJTF.getText()+"'";
            }
            if(xuan=="电话")
            {
                sql="select *  from yuangongtable where  number='"+this.tiaojianJTF.getText()+"'";
            }
            if(xuan=="月收入")
            {
                sql="select *  from yuangongtable where  paymonth='"+this.tiaojianJTF.getText()+"'";
            }
            if(xuan=="归属管理员编号")
            {
                sql="select *  from yuangongtable where  adminid='"+this.tiaojianJTF.getText()+"'";
            }
            ...
            while(rs.next())  //rs是结果集，rs.next()让指针往下一个个走
            {
                ...
                data[0]=rs.getString("yuangongid");
                data[1]=rs.getString("name");
                data[2]=rs.getString("sex");
                data[3]=rs.getString("age");
                data[4]=rs.getString("number");
                data[5]=rs.getString("paymonth");
                data[6]=rs.getString("ruzhidate");
                data[7]=rs.getString("adminid");
                dtm.addRow(data);
            }
            conn.close();
        }
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Selectyuangong().setVisible(true);
            }
        });
    }
